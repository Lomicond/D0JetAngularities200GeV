                                    ...
                                    const Char_t *outputFile = "output_D0JetSimTest_Neil", Int_t nEvents = 30, bool doTEST = kTRUE, int pYear = 2014) ///Zmenit to v xml!!!!
{
   ...
   TString out = outputFile;
    ...
    /**/HIOverlayMaker->setMcAbsEtaD0(true, 1); //false = not used, abs(eta) <= value
    /**/HIOverlayMaker->SetMcD0Mass(0.13957); //D0 = 1.86484, GAMMA = 0, PION_PLUS = 0.13957
    ...
    //Hadronic correction
    HIOverlayMaker->SetHadronicCorrFrac(1.); // fractional hadronic correction

    /**/HIOverlayMaker->setMaxDcaZHadronCorr(false, 3.0); // true = dca_z, false = dca; <= [cm]
    ...
    HIOverlayMaker->SetJetRad(0.4);        // jet radius
    ...
    HIOverlayMaker->setJetFractionNeutralToJetPt(	true, 		true, 		true,	    0.95    	); //
    HIOverlayMaker->setJetMinPt(			false, 		false, 		false,	    1       	); // >= [GeV/c]
    HIOverlayMaker->setJetMinArea(			false, 		false, 		false,	    0      	); // >= value*pi*R^2
    /**/HIOverlayMaker->setJetMinAbsEta(		true, 		false, 		false,	 1.-jetRadius   ); // abs(value) <= 


    //Strategy for jet background subtraction
    HIOverlayMaker->SetBgSubtraction(2); //0 - Area based, 1 - ICS, 2 - jet shape
    /**/HIOverlayMaker->setJetNHardestSkipped(2, 2); // First: 0-10%; Second: 10-80%
    //Set alphas and massive for background
    HIOverlayMaker->SetIBSMass(true);
    HIOverlayMaker->SetIBSAlpha1(0);
    HIOverlayMaker->SetIBSAlpha2(0);
