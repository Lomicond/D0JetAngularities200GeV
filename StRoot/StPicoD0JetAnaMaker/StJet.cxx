...
StJet::StJet() : //StVParticle(), // FIXME
                 ...
                 fGhosts(),
                 fJetConstit(),
	      fJetShapeCorrL10half(-999),
	      fJetShapeCorrL11(-999),
	      fJetShapeCorrL11half(-999),
 	      fJetShapeCorrL12(-999),
 	      fJetShapeCorrL13(-999),
	      fJetShapeCorrDisp(-999),                  
...
StJet::StJet(const StJet &jet) : //StVParticle(jet), // FIXME
                                 ...
                                 fJetShapeCorrL10half(jet.fJetShapeCorrL10half),
                                 fJetShapeCorrL11(jet.fJetShapeCorrL11),
                                 fJetShapeCorrL11half(jet.fJetShapeCorrL11half),
                                 fJetShapeCorrL12(jet.fJetShapeCorrL12),
                                 fJetShapeCorrL13(jet.fJetShapeCorrL13),
                                 fJetShapeCorrDisp(jet.fJetShapeCorrDisp), 
                                 fJetRho(jet.fJetRho),
                                 fJetRhoM(jet.fJetRhoM)
                                // ,fJetShapeProperties(0)
{
  // Copy constructor.
...
Double_t StJet::GetZ(const Double_t trkPx, const Double_t trkPy, const Double_t trkPz) const
{
  ...
  {
    cout << Form("%s: strange, pjet*pjet seems to be zero pJetSq: %f", "NaN", pJetSq);
    return -1;
  }
}
...
Double_t StJet::GetZD0(vector<TLorentzVector>& tracks) const
{
  ...
  {
    cout << Form("%s: strange, pjet*pjet seems to be zero pJetSq: %f", /*GetName()*/ "NaN", pJetSq);
    return -1;
  }
}

Double_t StJet::GetAngularityLambda(const Double_t &kappa, const Double_t &alpha, const Double_t &jetRadius) const
{
  Double_t pJet = Pt();
 ...
 // const vector<fastjet::PseudoJet> &constituents = fJetConstit;
  ...
  for (vector<fastjet::PseudoJet>::const_iterator particle = fJetConstit.begin(); particle != fJetConstit.end(); ++particle)
  {
    ...
    if (uid >= 0) //charged + D0
    {
      ...
      lambda += pow(particle->pt() / pJet, kappa) * pow(delta_R / jetRadius, alpha);
    }
  }
  return lambda;
}
/*
Int_t StJet::GenNumberOfCons(const bool ghost = false, const bool smallPtCut = true)const{
//ghost = true => consider even ghosts:
//smallPtCut = true => pt cut on real particles
  ...
  for (vector<fastjet::PseudoJet>::const_iterator particle = fJetConstit.begin(); particle != fJetConstit.end(); ++particle)
  {
    	 ...
    	 Int_t uid = particle->user_index();
    	 bool smallPt = smallPtCut && (particle->perp() < 0.001) && uid!=-1;
	 if ((!ghost || (uid != -1))&& !smallPt) counter++;
...
*/
Int_t StJet::GenNumberOfCons(const bool includeGhosts = false,const bool applySmallPtCut = false) const{

    Int_t counter = 0;

    for (const auto& p : fJetConstit) {

        const int uid = p.user_index();
        const bool isGhost = (uid == -1);
        const bool ptTooSmall = applySmallPtCut && (p.perp() < 0.001);


        if (!includeGhosts && isGhost) continue;     

        if (ptTooSmall) continue;        
...
Double_t StJet::GetJetRho() const{

return fJetRho;

}

Double_t StJet::GetJetRhoM() const{

return fJetRhoM;

}
/*
Double_t StJet::GetActualShapeL11() const{

return fJetShapeCorrL11;

}
Double_t StJet::GetActualShapeL11half() const{

return fJetShapeCorrL11half;

}
Double_t StJet::GetActualShapeL12() const{

return fJetShapeCorrL12;

}
Double_t StJet::GetActualShapeL13() const{

return fJetShapeCorrL13;

}

Double_t StJet::GetActualShapeL1half() const{

return fJetShapeCorrL1half;

}

Double_t StJet::GetActualShapeDisp() const{

return fJetShapeCorrDisp;

}
*/
Int_t StJet::GenNumberOfMCCons(const bool smallPtCut = true)const{
//ghost = true => consider even ghosts:
//smallPtCut = true => pt cut on real particles
  ...
  for (vector<fastjet::PseudoJet>::const_iterator particle = fJetConstit.begin(); particle != fJetConstit.end(); ++particle)
  {
    	 ...
    	 Int_t uid = particle->user_index();
    	 bool smallPt = smallPtCut && (particle->perp() < 0.001);
	 if (abs(uid) > 9999 && !smallPt) counter++;
...
void StJet::GetMomentumOfConst(TH2D* hMcJetConstMom)const{
  ...
  for (vector<fastjet::PseudoJet>::const_iterator particle = fJetConstit.begin(); particle != fJetConstit.end(); ++particle)
  {
	 ...
	 Double_t jetPt = Pt();
    	 ...
    	 if (mass >= 1) continue;
	 hMcJetConstMom->Fill(jetPt,particle->perp());
...
void StJet::GetThetaOfConst(TH2D* hMcJetConstTheta)const{
  ...
  for (vector<fastjet::PseudoJet>::const_iterator particle = fJetConstit.begin(); particle != fJetConstit.end(); ++particle){
    ...
    for (vector<fastjet::PseudoJet>::const_iterator particle = fJetConstit.begin(); particle != fJetConstit.end(); ++particle){
        	 Double_t mass = particle->m();
    	if (mass >= 1) continue;
 	double kx = particle->px();
  	double ky = particle->py();
  	double kz = particle->pz();
    	double dot = px*kx + py*ky + pz*kz;
	double norm_p = sqrt(px*px + py*py + pz*pz);
	double norm_k = sqrt(kx*kx + ky*ky + kz*kz);
	
	double cosTheta = dot / (norm_p * norm_k);
	double theta = acos(cosTheta);
	hMcJetConstTheta->Fill(jetPt,theta);
